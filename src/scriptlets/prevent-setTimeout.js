/**
 * UltraBlock Scriptlet: prevent-setTimeout
 * Based on: AdGuard Scriptlets / uBlock Origin
 * Wraps window.setTimeout to prevent calls where the callback matches
 ...
 */
function preventSetTimeout(args) {
  'use strict';
  ...
  window.setTimeout = function setTimeout(callback, delay) {
    ...
    if (shouldPrevent) {
      // Return a valid timer ID but don't schedule the callback
      return _origSetTimeout.call(window, function() {}, 0);
    }
